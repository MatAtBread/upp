...
@define RefPtr(T) {
    ...
    const detectCustomMethod = (methodSuffix) => {
        ...
        const defs = upp.query(`(function_definition declarator: (function_declarator declarator: (identifier) @name)) @def`, upp.root);
        if (defs.some(m => m.captures.name.text === methodName)) return true;
            ...
            try {
                ...
                const robustQuery = (pattern, node) => {
                     try {
                         ...
                         // If we assume the helper's language IS correct but the node is strict?
                         // Maybe we need to use `new helpers.TreeSitter.Query(helpers.Language, pattern)` manually?
                         // That's what helpers.query does.
                         if (node.tree && node.tree.language && helpers.TreeSitter) {
                             try {
                                 const q = new helpers.TreeSitter.Query(node.tree.language, pattern);
                ...
                if (typeText.startsWith('RefPtr_')) {
                    ...
                    if (isInit) {
                        const res = helpers.query('(init_declarator declarator: (_) @decl)', m.captures.decl);
                        if (res.length > 0) {
                    ...
                    if (scope) {
                        ...
                        const cleanupQuery = helpers.query(`(call_expression function: (identifier) @fn arguments: (argument_list (identifier) @arg))`, scope);
                        for (const match of cleanupQuery) {
                            if (match.captures.fn.text === `_release_${typeSuffix}` && match.captures.arg.text === varName) {
                        ...
                        const returns = helpers.query(`(return_statement) @ret`, scope);
                    ...
                    if (isInit) {
                         ...
                         // Safer: use query
                         const valMatch = helpers.query('(init_declarator value: (_) @val)', m.captures.decl);
                        ...
                        // Mangling: _${typeSuffix}_method_Create
                        ...
                        const createFn = helpers.query(`(function_definition) @def`, startNode).find(m => {
                            const defNode = m.captures.def;
                            // Search for the name identifier within the declarator subtree
                            const nameNode = helpers.query(`(function_declarator declarator: (identifier) @name)`, defNode);
